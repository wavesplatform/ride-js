...
describe('toInt',  () => {
    ...
    test.each([
        ...
        const contract = precondition.generateOnlyMatcherContract(version, testData, toIntFromByteVectorOnIndex);
        const compiled = compiler.compile(contract);
        expect(compiled.error).toBeUndefined();
    });
    ...
    test.each([
        ...
        const contract = precondition.generateOnlyMatcherContract(version, int, invalidToInt);
        const compiled = compiler.compile(contract);
        expect(compiled.error)
            .toContain(`Can't find a function overload 'toInt'()`);
    });
